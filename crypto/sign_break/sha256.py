...
def generate_hash(message: str | bytes | bytearray, debug=False, broken_h=None, broken_length=None) -> bytes:
    ...
    if broken_length is None:
        message += length.to_bytes(8, 'big')  # pad to 8 bytes or 64 bits
    else:
        message += broken_length.to_bytes(8, 'big')  # pad to 8 bytes or 64 bits

    assert (len(message) * 8) % 512 == 0, "Padding did not complete properly!"

    if debug:
        print('[DEBUG]', 'padded message', message)
        print('[DEBUG]', 'broken_h', broken_h)
        print('[DEBUG]', 'broken_length', broken_length)
    ...
    for i in range(0, len(message), 64):  # 64 bytes is 512 bits
        ...
        if debug:
            print('[DEBUG]', f'block {i}', message[i:i + 64])
    ...
    if broken_h is not None:
        h0, h1, h2, h3, h4, h5, h6, h7 = broken_h
